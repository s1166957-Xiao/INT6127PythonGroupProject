...
class AreaDialog:
    ...
    def load_area_list(self):
        ...
        try:
            ...
            for area in areas:
                # area[0]: area_id, area[1]: area_name, area[2]: capacity, 
                # area[3]: description, area[4]: current_items, area[5]: locations
                # 注意：当前数据库中没有"负责人"和"联系电话"字段，使用空字符串占位
                self.tree.insert("", tk.END, values=(
                    area[0] if len(area) > 0 else "",  # 区域ID
                    area[1] if len(area) > 1 else "",  # 区域名称
                    "",  # 负责人（数据库中没有此字段）
                    "",  # 联系电话（数据库中没有此字段）
                    area[3] if len(area) > 3 else ""   # 备注（使用description字段）
                ))
        except Exception as e:
            messagebox.showerror("错误", f"加载区域列表失败：{str(e)}")
