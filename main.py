...
def change_theme(self, theme_name):
    """一键切换主题（增加异常处理以防主题不存在）"""
    try:
        if hasattr(self.root, "set_theme"):
            self.root.set_theme(theme_name)
            messagebox.showinfo("提示", f"已切换主题: {theme_name}")
        else:
...
def qr_read(self):
    ...
    try:
        ...
        self.location_combobox.insert(0, info.get('location', ''))
...
# 下面这些模块级函数本应作为 `ExpressManagementSystem` 的方法定义。
# 为兼容当前文件结构，将它们绑定到类上，避免 `AttributeError`。
_method_names = [
...
for _name in _method_names:
    if _name in globals() and not hasattr(ExpressManagementSystem, _name):
        setattr(ExpressManagementSystem, _name, globals()[_name])
