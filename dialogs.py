...
class AdminLoginDialog:
    ...
    def __init__(self, parent, totp_manager):
        self.dialog = tk.Toplevel(parent)
    ...
    def show_totp_setup(self):
        """显示TOTP设置说明和二维码"""
        # 生成新的TOTP密钥和二维码
        qr_path, _ = self.totp_manager.generate_qr_code()
        
        # 创建并显示设置说明窗口
        setup_dialog = tk.Toplevel(self.dialog)
        setup_dialog.title("首次登录配置")
...
注意：请妥善保管您的验证器应用，它是登录系统的必要工具。
        ...
        try:
            ...
            ttk.Label(setup_dialog, text=f"无法加载二维码图片: {str(e)}", 
                     foreground="red").pack(pady=20)
        
        # 确认按钮
        ttk.Button(setup_dialog, text="我已完成设置", 
                  command=setup_dialog.destroy).pack(pady=20)
